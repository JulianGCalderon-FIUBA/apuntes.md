...
Afortunadamente, estas dificultades pueden ser resueltas utilizando un ***Virtual Local Area Network (VLAN)***. Un ***switch*** que permite VLAN permite que multiples VLAN sean definidas por sobre una única LAN física. En una ***port-based VLAN***, los puertos del ***switch*** central son distribuidos entre los distintos grupos por el administrador de red (cada puerto es asociado a una VLAN distinta).
